...
- [ ] T020 [US1] Implement `OllamaSidecar::stop(&self, grace)` in `manager.rs`: sends SIGTERM to the child PID, waits up to `grace`, sends SIGKILL on timeout. Updates `self.status` to `Stopping` then `Stopped`. **Open implementation question**: current `stop()` calls `CommandChild::kill()` (immediate SIGKILL) and ignores the `grace` argument (named `_grace`). Behavior is acceptable in practice — T023 confirms the port is released within 5 s — but diverges from the SIGTERM-first wording above. Either implement the graceful-then-forceful path or amend this task to reflect the SIGKILL-only choice.
