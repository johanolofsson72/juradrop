...
- Email with Swedish chars (`åsa@exempel.se`) — PREMISE CORRECTED during implementation: Rust's regex crate has Unicode-aware `\w` by default, so the spec-014 pattern already matched å-emails in full (the finding's "ASCII-only \w" premise came from other regex engines' semantics). Resolution kept in spirit (FR-009): the local-part char class is made EXPLICIT (`[\wåäöÅÄÖ.+-]`) and a pinning test asserts the full å-email match, so the guarantee survives any future engine/flag change.
