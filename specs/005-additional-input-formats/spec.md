...
- **FR-004**: System MUST detect PDFs where `pdf-extract` returned **zero raw bytes** (image-only scanned PDFs with no embedded text content stream at all, despite having ≥ 1 page) and map them to a NEW `NoExtractableText` Swedish error: "Hittade ingen text att läsa i PDF-filen — skannade bilder stöds inte än." No sidecar is written. OCR is explicitly out of scope. `NoExtractableText` is PDF-only and fires BEFORE any trim — `pdf-extract` returning some bytes that are whitespace-only after trim does NOT count as `NoExtractableText`; that case maps to the existing `EmptyText` error from spec 003 (which already applies to all four formats).
