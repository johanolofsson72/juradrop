...
- Q: Test mocking strategy for the Ollama HTTP layer? → A: **Hand-rolled HTTP mock, no new dep.** A 50-line `tokio::net::TcpListener` + `httparse` (already transitive via tauri) listener that binds to `127.0.0.1:0` (random port), serves a deterministic `/api/generate` response, and tracks request bodies for assertions. Wiremock-rs would work but adds a heavyweight dep tree. Hand-rolled mock matches spec 011's "no new deps" discipline.
...
- **FR-003**: `src-tauri/tests/fixtures/zone-identity.json` MUST gain 3 new entries with `slug`, `title`, `hint_copy`, `sidecar_suffix`, `has_disclaimer_paragraph` fields per existing schema. Specifically:
  ...
  - Generera: title `Generera juridisk text`, slug `generera`, sidecar suffix `.generera`, **HAS DISCLAIMER** (AI-genererad text — kontrollera mot källa).
  - Kallor: title `Källförteckning`, slug `kallor`, sidecar suffix `.kallor`, no disclaimer.
- **FR-004**: `App.tsx` grid CSS classes MUST update from `lg:grid-cols-3` (which produces 2×3 with 6 zones) to keep `lg:grid-cols-3` (producing 3×3 with 9 zones — same column count, more rows). The `sm:grid-cols-2` breakpoint and `grid-cols-1` mobile fallback remain as-is.

#### Constitution amendment
- **FR-005**: `.specify/memory/constitution.md` Principle VII MUST be amended. Current text: "The Settings panel MAY expose model selection ('Snabb / Smart / Stor') but never raw model tags like `llama3.2:3b-instruct-q4_K_M`." (no zone-count claim in VII directly, but the document elsewhere references "six zones"). Search the constitution for every occurrence of `six zones`/`six drop zones`/`2×3`/`6 zones` and update to `nine zones`/`3×3`/`9 zones`. Version bump 1.0.0 → 1.1.0 (MINOR — "material expansion" per Governance).
...
- **FR-011**: 9 new Rust integration tests at `src-tauri/tests/zone_pipeline_<zone>.rs` (one per zone). Each:
  - Spawns the hand-rolled HTTP mock on `127.0.0.1:0` with a deterministic `/api/generate` response.
  - Sets `JURADROP_OLLAMA_URL` env var to the mock's URL (test seam from spec 013-withdrawn earlier needs to be added: in debug builds only, `src-tauri/src/sidecar/client.rs` reads this env var to override the default).
  - Opens the zone's fixture document.
  - Calls the zone's dispatch function (decomposed from `handle_drop` if needed).
...
- **FR-013**: Walk every `#[ignore]`'d test in `src-tauri/tests/*.rs`. For each:
  - If un-ignorable with the new mock harness → un-ignore + verify it passes.
  - If genuinely hardware-required → keep `#[ignore]`'d + add one-line `// HARDWARE: <reason>` comment.
...
- **SC-001**: All 9 zones present in the rendered grid after spec 013 ships. Verified by a vitest test asserting `MODEL_TIERS.length === 9` (or equivalent zone-count assertion via ZONE_ORDER).
...
- **SC-003**: 7 extraction-probe tests pass on every `cargo test`. Each format extracts the canonical paragraph (or returns the spec-009-defined named-format error for `.pages`).
