...
- [ ] T006 [P] Rename `TRUNCATION_CHAR_LIMIT` → `EXTRACT_CEILING_CHARS = 288_000` in `src-tauri/src/zones/extract.rs` (doc comment: coarse MEMORY bound only — chunking owns the user-facing 12-chunk cap per analyze F1; was_truncated = exceeded the memory bound); update the re-export in `src-tauri/src/zones/docx_extract.rs:21` and the pinning tests in `src-tauri/tests/zone_docx_robustness.rs:90-119`
...
- [ ] T009 [US1] Implement combine step in `src-tauri/src/zones/sammanfatta.rs`: emit "Sammanställer…" snapshot; `Reduce` → label partials "Del {i}:" + one framed combine generate (raced vs cancel; recursive re-chunk safety net if combine input > CHUNK_CHAR_TARGET); `CondenseThenStructure` → per-chunk passes already used STRUKTURERA_CONDENSE, then one framed strukturera pass over joined condensates (same recursive re-condense safety net when the joined condensates exceed CHUNK_CHAR_TARGET — analyze F2); `Concat`/`Aggregate` → `merge_concat`/`merge_aggregate` (no model call); any combine error → `finalize_with_failure(ModelError)`
