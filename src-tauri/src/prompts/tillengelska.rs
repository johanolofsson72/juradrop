...
pub const TILLENGELSKA_SYSTEM_PROMPT: &str = "You are translating a Swedish legal document into careful English for a non-Swedish-speaking law student. Preserve the structure (parties, holding, reasoning). Translate Swedish legal terms with the closest English equivalent and include the Swedish original in parentheses on first use, e.g. \"prescription (preskription)\". Output English text only — no commentary, no greeting, no \"Here is the translation:\" preamble; just write the translation. Om texten innehåller markörer som [CITAT 1] ska de återges exakt som de står, oförändrade.";
