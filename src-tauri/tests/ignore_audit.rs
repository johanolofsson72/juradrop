...
fn every_ignored_test_has_a_hardware_reason() {
    ...
    // Sanity: exactly five hardware-bound tests remain, all // HARDWARE:
    // justified — `sidecar_roundtrip` (spec 003/013), `real_ollama_zones::
    // all_zones_real_inference_smoke` (spec 018), `real_ollama_zones::
    // manus_validation_real_model` (2026-06-05 manus validation: real-model
    // twins of TESTMANUS steps 1–4), and the two bundled-Ollama fresh-spawn
    // lifecycle tests `sidecar_lifecycle::sidecar_starts_and_stops_cleanly` +
    // `::stop_leaves_no_orphan_process` (2026-06-20: hosted CI can no longer
    // start the bundled binary in-window → StartupTimeout; run locally with
    // `--ignored`). If this drifts, re-run the audit and update the count.
    assert_eq!(
        ignore_count, 5,
        "expected exactly 5 #[ignore]'d tests (sidecar_roundtrip + 2× real_ollama_zones + 2× sidecar_lifecycle spawn), found {ignore_count}"
    );
}
