...
4. **Adversarial review agent.** Beyond the normal `/code-review`, run a dedicated adversarial pass — the `security-scanner` agent in "assume it's exploitable, prove me wrong" mode, plus a `dotnet-reviewer` / language reviewer pass focused on the new trust boundaries. Add Claude Code's built-in **`/security-review`** (reviews the pending changes on the current branch) as a second, project-agnostic opinion: it does not know our conventions, which is exactly why it catches what an agent primed on them talks itself past. Two independent passes, not one pass run twice. Treat every flagged item as a finding requiring an explicit fix/defer/dismiss decision per `.claude/rules/validation-followup.md`.
...
- `spec-interview.md` — the hardened triggers here are the **strong prior** for flagging a spec's interview for human overflow questions. In the default AUTO interview mode Claude auto-answers the base 15–25, but a spec that crosses any hardened trigger should almost always get the human-answered overflow questions (biased toward the threat-surface: authz, input tampering, information disclosure, resource exhaustion) — the human-side complement to this rule's automated threat-model pass.
